...
public class LanguageConverter{
	...
	{
		if (src.indexOf("/*LANG:") < 0) {return src;}
		...
		try {
			...
			if (dat.equals("")){return src;}
			String clss = ls.ssplit("CLASS",dat);
			Class c=null;
			if (lhash.containsKey(clss)) {c= (Class)lhash.get(clss);}
			else{
	...
	public static void main(String[] args)
	{
		...
		try {BufferedReader br = new BufferedReader(new FileReader(new File("languages.txt")));
			String s;
			while ((s=br.readLine())!=null)
			{
		...
		catch (Exception e) {System.out.println("Failed at reading languages file");System.exit(0);}
		...
		catch(Exception ez) {System.out.println("This language class seems to be corrupted, or did not subclass dscript.intl.LanguageConverter\n");
					System.exit(0);
		}
